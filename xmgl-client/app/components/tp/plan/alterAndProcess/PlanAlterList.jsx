...
import PlanAlterInfo, {createPlanAlterInfoModel} from './PlanAlterInfo';
import '../../tp.scss';
...
const createPlanAlterListModel  = function (modelName){
    mirror.model({
        name:modelName,
        initialState:{
        ...
        effects: {
            //查询获取数据条
            async getList(planId, getState) {
                actions[modelName].handleChange({'planId': planId})//把计划id塞入queryOptions，传递到后台判断。
                let queryOptions = getState()[modelName].queryOptions
                let response = await getJson('/api/plan/getPlanAlterList', {
                    body: {
            ...
            async editInfo(data, getState){
                ...
                if (!actions[modelName]){
                    createPlanAlterInfoModel(modelName);
                }
                let content = (<PlanAlterInfo id = {data.id} modelName = {modelName}/>)
                actions.tabsTmpl.addTab({key:modelName,name :'计划变更详情','content':content})
            }
        }
    });
    render()
...
const PlanAlterList = connect((state,ownProps)=>{
    let planId=ownProps.planId;  //计划详情传递的计划id
    let modelName = ownProps.modelName;
    if(state[modelName].forceUpdate){
    actions[modelName].getList(planId)//根据传递过来的计划id，直接获取数据。
    }
...
export default PlanAlterList;
export {createPlanAlterListModel};