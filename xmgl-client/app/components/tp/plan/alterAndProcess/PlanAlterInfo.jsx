...
import {Form, Row, Col, message, Input, Select, Button, Icon, Radio, Table , Upload, Column} from 'antd';
...
const createPlanAlterInfoModel = function (modelName) {
    mirror.model({
        name: modelName,
        initialState:{
        ...
        effects:{
            ...
            async loadAlterData(data,getState) {
                let alterInfoList = await getJson('/api/plan/getPlanAlterInfoById/' + data.id, {'method': 'GET'});
...
const PlanAlterInfo = connect((state, ownProps) => {
                ...
                <Row  type="flex" justify="space-around" >
                    <Col span={5}>
                        <FormItem {...formItemLayout} label={`计划名称：`}>
                            <Input
                                value={props.alterData.plan_name}
                            />
                        </FormItem>
                    </Col>
                    <Col span={5}>
...
export default PlanAlterInfo
export {createPlanAlterInfoModel};