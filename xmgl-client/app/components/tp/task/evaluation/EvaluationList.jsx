...
import '../../tp.scss';
import CreateEvalution, {createCreateTaskEvalModel} from './CreateEvaluation'
...
const evaluationListModel = function (modelName) {
    mirror.model({
        name: modelName,
        initialState: {
            list: undefined,
                    ...
                    formatter: function (row, data, index) {//显示操作按钮
                        return (
                            <div className="list-item-action">
                                ...
                                <span onClick={() => {
                                    actions[modelName].deleteEvaluation({
                                        'id': row.id
                                    })
        ...
        effects: {
            async getList(data, getState) {
                ...
                if(typeof(data.curTaskId) != "undefined"){
                    ...
                    response = await getJson('/api/task/getEvaluationListByTaskId/' + curTaskId,
                        {
                            body: {
                                queryOptions: queryOptions,
                                curPage: curPage,
                    ...
                    response = await getJson('/api/task/getEvaluationListByEmployeeId/' + taskId + '/' + curEmployeeId,
                        {
                            body: {
                                queryOptions: queryOptions,
                                curPage: curPage,
                    ...
                    response = await getJson('/api/task/getEvaluationList',
                        {
                            body: {
                                queryOptions: queryOptions,
                                curPage: curPage,
            ...
            async deleteEvaluation(data, getState){
                let response = await getJson('/api/task/deleteEvaluation', {
                    body: {
                        id: Number(data.id)
                    }
                })
            ...
            async editInfo(data, getState){
                ...
                let content = (<CreateEvalution evalId={data.id} modelName={evalModelName}/>);
...
const EvaluationList = connect((state, ownProps) =>{
    let modelName = ownProps.modelName;
    if (state[modelName].forceQuery) {
        actions[modelName].getList({curEmployeeId:ownProps.curEmployeeId,taskId:ownProps.taskId,curTaskId:ownProps.curTaskId})
    }
