...
/*const modelName = "employeeEvaluateResult";*/

const employeeEvaluateResultModel = function (modelName){
mirror.model({
    name: modelName,
    initialState: {
        list: undefined,
                ...
                formatter: function (row, data, index) {//显示操作按钮
                    return (
                        <div className="list-item-action">
                                <span onClick={() => {
                                    actions[modelName].editInfo({'employee_id':row.employee_id,'task_id':row.task_id})
    ...
    effects: {
        async getList(data, getState) {
            ...
            if(typeof(employeeId)!="undefined"){
                response = await getJson('/api/task/getEmployeeEvaluateResultListByEmployeeId/'+employeeId+'/'+data.curTaskId,
                    {
                        body: {
                ...
                response = await getJson('/api/task/getEmployeeEvaluateResultList',
                    {
                        body: {
                            queryOptions: queryOptions,
                            curPage: curPage,
        ...
        async editInfo(data,getState){
            ...
            let content = (<EvaluationList curEmployeeId={data.employee_id} taskId={data.task_id} modelName = {name}/>);
            actions.tabsTmpl.addTab({key:name, name:`员工考评详情`,'content':content});
        },
    }
})
    render();
...
const EmployeeEvaluateResult = connect((state,ownProps) => {
    let modelName = ownProps.modelName;
    if (state[modelName].forceQuery) {
        actions
            [modelName]
            .getList({employeeId:ownProps.employeeId,curTaskId:ownProps.curTaskId})
    }
...
export default EmployeeEvaluateResult
export {employeeEvaluateResultModel};