            ...
            <div class="panel-heading">
                <div class="panel-options">
                    <ul class="nav nav-tabs">
                        <li class="active"><a href="project_detail.html#tab-1" data-toggle="tab" aria-expanded="true">谁评论了我</a>
                        </li>
                        <li class="">
                            <a href="project_detail.html#tab-2" data-toggle="tab" aria-expanded="false">我评论了谁</a>
                        </li>
                    </ul>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    function getReceivePage(){
        var url = "${root}/manage/message/getMessageList?receiver_id=" + $("#userId").val() + "&messageType=PL";
    ...
    function getSendPage(){
        var url = "${root}/manage/message/getMessageList?sender_id=" + $("#userId").val() + "&messageType=PL";
        ...
        if (response && response.length) {
            for (var i = 0; i < response.length; i++) {
                ...
                html += '<div class="feed-element">' +
                    ...
                    '<a><span onclick="checkMessage(\'' + response[i].business_type + '\',' + response[i].business_id + ',' + response[i].id + ',\'' + response[i].sender_id + '\')"><strong>' + response[i].sender + '</strong>评论了<strong>我</strong>@<strong>' + response[i].business_name + '</strong></span></a>' +
                    '<br>' +
        ...
        if (response.length == 0) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                html += ' <div class="feed-element">' +
                    ...
                    '<div class="media-body ">' +
                    '<small class="pull-right text-navy">' + response[i].timeDiff + '&nbsp;' + response[i].remind_time + '</small>' +
                    '<a><span onclick="checkMessage(\'' + response[i].business_type + '\',' + response[i].business_id + ',' + response[i].id + ',\'' + response[i].sender_id + '\')"><strong>我</strong>评论了<strong>' + response[i].receiver + '</strong>@<strong>' + response[i].business_name + '</strong></span></a>' +
                    '<br>' +
                    '<div class="well">我评论了' + response[i].receiver + '，@【' + response[i].business_name + '】{ ' + response[i].log_date + ' } </div>' +
                    '</div> </div>';
            }
        }
    ...
    function checkMessage(log_type, log_id, message_id, sender_id) {
        ...
        else if (log_type == "MZJH") {
            url1 = "${root}/manage/weekLog/weekLogInfoIndex/" + log_id+"/comments/MZJH";
        }
        else if (log_type == "MYJH") {
            url1 = "${root}/manage/monthLog/monthLogInfo/" + log_id+"/comments/MYJH";
        }
...
<script src="${root}/res/hplus/js/plugins/metisMenu/jquery.metisMenu.js"></script>
<script src="${root}/res/hplus/js/hplus.js?v=4.1.0"></script>
<script src="${root}/res/hplus/js/contabs.js"></script>
</html>
