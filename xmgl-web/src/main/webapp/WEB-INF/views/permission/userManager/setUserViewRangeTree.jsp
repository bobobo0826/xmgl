...
<body  >
	...
	<div class="col-sm-12">
					...
					<div class="col-md-10" >
						 <div class="ibox float-e-margins" > 
							<iframe id="userList" name="userList" frameborder=0  style='width:100%; height:1500px; ' src=""></iframe>
						</div>
					</div>
		</div>
	</div>
	...
	<script type="text/javascript"> 
	    ...
	    var tool={
				...
				{ 
					...
					frame.src = "${root}/permission/indexSetUserViewRangeList.action?_deptId=" + treeNode.id;
				},
	    ...
	    {
	    	url = "${root}/permission/initaddUserInfo.action";
	        width = "90%";
 	        height = "80%";
	    ...
	    {
	    	url = "${root}/permission/initUserDetail.action?id="+id;
	        width = "90%";
 	        height = "80%";
	    	f_open("editUserInfo", "用户信息编辑", width, height, url, true);
	    }
	    ...
	    {
	    	_curCanVIewUserMap.put(key,value);
	    }
	    function remove(key)
	    {
	    	_curCanVIewUserMap.remove(key);
	    }
	    function doComfirmSelect()
	    {
	    	var array=_curCanVIewUserMap.getArr("user_id","display_name");
	    	parent.doConfirmSelectUser(JSON.stringify(array));
	    	f_close("selectUsers");
	    }
	    function getCurCanVIewUserArrary()
	    {
	    	var array=_curCanVIewUserMap.getArr("user_id","display_name");
	    	return array;
	    }
    ...
    <script type="text/javascript">
		var _zTree,_pNode;
		var _curCanVIewUserMap=new Map();
		$(document).ready(function(){
			...
			var curCanViewUserInfos=parent.doGetCurCanViewUserInfos();
			 if(!isEmpty(curCanViewUserInfos))
			 {
				 var array=JSON.parse(curCanViewUserInfos);
				 _curCanVIewUserMap.putArray(array,"user_id","display_name");  
			 } 
