...
<div class="inner6px" >
    <table  border=0 height=600px align=left>
        <tr>
            ...
            <td width=2000px align=left valign=top>
...
<div id="rMenu">
    <ul>
        <li id="m_add" onclick="tool.addSubNode();">添加规范</li>
        <li id="m_del" onclick="tool.delNode();">删除规范</li>
        <li id="m_refresh" onclick="tool.refresh();">刷新</li>
        <%--<li id="m_publish" onclick="tool.publish();">发布</li>--%>
    </ul>
</div>
    ...
    var tool={
        onClick:function(event, treeId, treeNode)
        {
            ...
            if(!treeNode.load)
            {
                ...
                var url = '${root}/manage/writingStandards/queryStandardsList';
            ...
            if(treeNode.id != -1&&treeNode.id!=1 )
                frame.src = "${root}/manage/writingStandards/standardsInfoIndex?_id=" + treeNode.id;
        ...
        delNode:function(){
            ...
            layer.confirm("是否确定删除该角色？" ,{
                ...
                data = {'_id':_pNode.id};
                var url = '${root}/manage/writingStandards/delStandardsInfo';
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    data:data,
        ...
        addSubNode:function(){
            console.log(_pNode)
            if(_pNode.pid!='-1'&&typeof(_pNode.pid)!="undefined"){
                layer.msg("该节点不能添加子节点");
                tool.hideRMenu();
                return;
            }
            tool.hideRMenu();
            var frame = document.getElementById('menuItemIndex');
            frame.src = "${root}/manage/writingStandards/standardsInfoIndex?_parentId=" + _pNode.id;

        },
        publish:function () {
            tool.hideRMenu();
            layer.confirm("是否确定发布该规范？" ,{
                btn: ['确定','取消'],
                shade: false //不显示遮罩
            },function(index){
                data = {'_id':_pNode.id};
                var url = '${root}/manage/writingStandards/publishStandards';
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    data:data,
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        var root = {'name':'规范手册','id':-1,open:true,isParent:true};
