...
function menuItem() {
	...
	if (flag) {
		...
		var str1 = '<iframe class="J_iframe" name="iframe' + dataIndex
				...
				+ '" seamless allowfullscreen mozallowfullscreen msallowfullscreen webkitallowfullscreen ></iframe>';
...
//刷新指定iframe
function refreshTabByid(dataId) {
...
function addTab(title,url,icon) { 
	var o = url, m = url, l = title, k = true;
...
//关闭选项卡菜单 
function h_tab() {
	var m = $(".J_menuTab.active").data("id");
	var l = $(".J_menuTab.active").width(); 
	if ($(".J_menuTab.active").next(".J_menuTab").size()) {
		var k = $(".J_menuTab.active").next(".J_menuTab:eq(0)")
				.data("id");
		$(".J_menuTab.active").next(".J_menuTab:eq(0)")
				.addClass("active");
		$(".J_mainContent .J_iframe").each(function() {
			if ($(this).data("id") == k) {
				$(this).show().siblings(".J_iframe").hide();
		...
		$(".page-tabs-content .J_menuTab.active").each(function() {
			if ($(this).data("id") == m) {
				$(this).remove();
				return false
			}
	...
	if ($(".J_menuTab.active").prev(".J_menuTab").size()) {
		var k = $(".J_menuTab.active").prev(".J_menuTab:last")
				.data("id");
		$(".J_menuTab.active").prev(".J_menuTab:last").addClass(
				"active");
		$(".J_mainContent .J_iframe").each(function() {
			if ($(this).data("id") == k) {
				$(this).show().siblings(".J_iframe").hide();
